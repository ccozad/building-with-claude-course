...
class PromptEvaluator:
    def __init__(self, max_concurrency=5, chat_model=None):
    ...
    def generate_unique_ideas(self, task_description, prompt_inputs_spec, num_cases):
        ...
        messages = []
        self.chat_model.add_user_message(messages, rendered_prompt)
        self.chat_model.add_assistant_message(messages, "```json")
        text = self.chat_model.chat(
            messages,
            stop_sequences=["```"],
    ...
    def grade_output(self, test_case, output, extra_criteria):
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        messages = []
        self.chat_model.add_user_message(messages, eval_prompt)
        self.chat_model.add_assistant_message(messages, "```json")
        eval_text = self.chat_model.chat(
            messages,
            stop_sequences=["```"],
            temperature=0.0,
        )
